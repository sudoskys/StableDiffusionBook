...
人类的语言是任意的和不精确的(除了 emoji 指代)，人类的语言从未进化到可以详细描述空间信息的程度。

许多赌徒遵循他们特定的某些仪式性行为的模式，认为这种模式会增加有利结果的机会。这种类型的假阳性模式不仅在现实中不起作用，而且还会不断地强化和表现出来，促使一个人在错误的方向上越走越远。
...
解析器理解事物的方式是不透明的，所以无法确切知晓词汇顺序是否具有 “锚点” 的作用。
